...
/**
 * Service for device states.
 */

@Singleton
public class DeviceStateServiceImpl extends DeviceServiceAbstract
        implements DeviceStateService, StateTopicEventHandler {
    ...
    public void onStateMessage(final PubsubMessage pubsubMessage, final AckReplyConsumer consumer) {
        ...
        repository.createNew(mapper.pubSubMessageToDeviceStateEntity(pubsubMessage))
                ...
                .onFailure(err -> log.error("Can't save state in database: {}", err.getMessage(), err));
    }
}
